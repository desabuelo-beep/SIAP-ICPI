...
with st.sidebar:
    ...
    st.markdown("---")
    # Estado Gemini en sidebar
    if GEMINI_ACTIVO:
        st.markdown("""
        <div style='background:#f0fff4; padding:0.7rem; border-radius:8px; text-align:center; border:1px solid #68d391;'>
            <div style='font-size:0.7rem; color:#276749;'>🤖 GEMINI AI</div>
            <div style='font-size:0.85rem; font-weight:700; color:#276749;'>Activo</div>
        </div>
        """, unsafe_allow_html=True)
    else:
        st.markdown("""
        <div style='background:#fffbeb; padding:0.7rem; border-radius:8px; text-align:center; border:1px solid #d69e2e;'>
            <div style='font-size:0.7rem; color:#744210;'>🤖 GEMINI AI</div>
            <div style='font-size:0.85rem; font-weight:600; color:#744210;'>Sin key</div>
        </div>
        """, unsafe_allow_html=True)
    st.markdown("---")
...
# ─────────────────────────────────────────────
    ...
    if modo == "Ver análisis del caso piloto: Montecristi 2024":
        ...
        try:
            ...
            st.info("📦 Agrega 'plotly' al requirements.txt para activar los gráficos interactivos.")
            st.dataframe(df_metas[["Meta", "ICPI%", "AVEP"]].style.background_gradient(subset=["ICPI%"], cmap="RdYlGn"))
...
*Generado por TERRA CIUDADANA | QUADRUM GovTech | {datetime.datetime.now().strftime('%d/%m/%Y')}*
        ...
        if financiador in argumentos:
            argumento_texto = argumentos[financiador]
            st.markdown(argumento_texto)
        elif GEMINI_ACTIVO:
            with st.spinner(f"🤖 Gemini generando argumento para {financiador}..."):
                _prompt_arg = f"""Eres un experto en cooperación internacional y financiamiento para gobiernos locales de Ecuador.
Genera un argumento técnico de elegibilidad para el GAD Municipal de Montecristi (Ecuador) ante: {financiador}

Datos verificados del GAD (Sistema TERRA SIAP-ICPI v2.0):
- ICPI global: 72.93% — Gestión por Mandato
- ICPI Gobernanza (ODS 16-17): 58.8% — Transición Crítica
- ICPI Infraestructura (ODS 6-9-11-13): 71.3% — Gestión por Mandato
- ICPI Inclusión Social (ODS 1-3-4-5-10): 63.5% — Transición Crítica
- Brecha declarado vs. real: 27.07 puntos porcentuales
- Acueducto CAF USD $28M aprobado diciembre 2024
- Modernización administrativa: ERP USD $252k + Maquinaria USD $600k

El argumento debe tener máximo 300 palabras, incluir datos específicos del GAD, 
mencionar los ODS alineados con la misión del organismo {financiador},
...
Termina siempre con: "Generado por TERRA CIUDADANA | QUADRUM GovTech | {datetime.datetime.now().strftime('%d/%m/%Y')}" """
                try:
                    ...
                    argumento_texto = f"**Error Gemini:** {_e}\nGAD Montecristi | ICPI: 72.93% | QUADRUM GovTech"
                    st.error(argumento_texto)
        else:
            argumento_texto = f"""**Argumento de elegibilidad — {financiador}:**
...
Generado por TERRA CIUDADANA | QUADRUM GovTech
    ...
    with modo_accion[1]:
        ...
        if necesidad and st.button("🔍 Analizar y vincular al PDOT"):
            ...
            if GEMINI_ACTIVO:
                with st.spinner("🤖 Gemini está analizando tu necesidad..."):
                    _prompt_modb = f"""Eres TERRA CIUDADANA, un asistente experto en gestión pública de GADs ecuatorianos.
Un ciudadano ha descrito la siguiente necesidad de su comunidad:
"{necesidad}"

Las metas del PDOT 2023-2027 del GAD Municipal de Montecristi con su ICPI son:
- AH-C-X-01 Agua Potable (ICPI: 72%) - Dir. Agua
- AH-C-X-02 Alcantarillado y PTAR (ICPI: 72%) - Dir. Agua
- AH-I-X-01 Vialidad Cantonal (ICPI: 58.3%) - Dir. Obras
- AH-I-X-02 Señalización (ICPI: 0%) - Dir. Obras
- AH-I-X-03 Equipamientos Públicos (ICPI: 58.3%) - Dir. Obras
- AH-I-N-01 Vivienda Interés Social (ICPI: 22.5%) - EP Montehogar
- SC-I-N-01 Salud Municipal (ICPI: 95%) - Patronato
- SC-L-N-02 TICs y Educativo (ICPI: 85%) - Dir. TIC
- SC-I-N-03 Grupos Prioritarios (ICPI: 53.4%) - Patronato
- FA-C-X-01 Desechos Sólidos (ICPI: 80%) - EP Aseo
- FA-I-X-01 Gestión del Riesgo (ICPI: 65%) - Dir. Ambiental
- FA-I-X-02 Áreas Verdes (ICPI: 68.8%) - Dir. Ambiental
- FA-L-N-01 Fauna Urbana (ICPI: 77%) - Dir. Ambiental
- EP-L-N-01 Fortalecimiento Productivo (ICPI: 95%) - Dir. Económica
- EP-L-X-01 Turismo (ICPI: 85%) - Dir. Turismo
- PI-I-G-01 Modernización Administrativa (ICPI: 85%) - Dir. Administrativa
- PI-I-G-02 PDOT/Catastro/Trámites (ICPI: 85%) - Dir. Planificación
- PI-L-G-01 Participación Ciudadana (ICPI: 53.4%) - Dir. Planificación
- SC-L-G-01 Cultura y Patrimonio (ICPI: 47.8%) - Dir. Cultura
- AH-I-X-04 Tránsito y Seguridad Vial (ICPI: 40.5%) - Dir. Obras

Responde en español, de forma clara y empática para un ciudadano sin formación técnica:
1. Identifica la(s) meta(s) del PDOT que se relacionan con su necesidad
2. Indica el ICPI actual y qué significa para el ciudadano
...
4. Mantén un tono empático y empoderador."""
                    try:
                        ...
                        st.info("Revisa tu API key en los secrets de Streamlit.")
            else:
                # Fallback sin Gemini
                ...
                elif "vía" in necesidad.lower() or "calle" in necesidad.lower() or "vial" in necesidad.lower():
                ...
                elif "basura" in necesidad.lower() or "desecho" in necesidad.lower():
                    st.success("✅ Tu necesidad se vincula con: **FA-C-X-01 — Desechos Sólidos** (ICPI: 80%)")
                    st.info("Esta meta tiene buen nivel de ejecución. Puedes solicitar un informe de cobertura.")
                else:
                    st.info("Activa la integración con Gemini AI para un análisis inteligente de tu necesidad.")
                    st.caption("Agrega tu GEMINI_API_KEY en los secrets de Streamlit para activar el análisis IA.")
...
# ─────────────────────────────────────────────
    ...
    **Asistentes:** {eval_data['asistentes']} | **Informe N°:** {eval_data['informe_numero']}
    ...
    if GEMINI_ACTIVO:
        st.markdown("---")
        st.markdown("### 🤖 Tribunal IA — Análisis de Fricción Narrativa")
        if st.button("🔎 Activar análisis Gemini del Tribunal", use_container_width=True):
            with st.spinner("Gemini analizando el discurso, el informe técnico y los datos TERRA..."):
                _hallazgos_str = "\n".join([
                    f"- {h['tipo']}: {h['meta']} | Discurso: {h['discurso']} | CPCCS: {h['informe']} | TERRA: {h['terra']}"
                    for h in eval_data["hallazgos"]
                ])
                _promesas_str = ", ".join(eval_data["promesas_sin_pdot"])
                _prompt_tribunal = f"""Eres un experto en transparencia pública y gestión municipal ecuatoriana.
Analiza la siguiente información sobre la Rendición de Cuentas 2024 del GAD Municipal de Montecristi (Alcalde: Ing. Jonathan Toro):

DATOS VERIFICADOS POR TERRA:
- ICPI real verificado: 72.93% (Gestión por Mandato)
- ICM declarado oficialmente en SIGAD: 100%
- Brecha entre declarado y real: 27.07 puntos porcentuales
- Coeficiente de Fricción Narrativa: {eval_data['friccion_narrativa']}%
- Coherencia verificada: {eval_data['coherencia']}%

HALLAZGOS META A META:
{_hallazgos_str}

PROMESAS CNE SIN META PDOT:
{_promesas_str}

Escribe un dictamen técnico-ciudadano de máximo 400 palabras que:
1. Evalúe la coherencia entre el discurso político, el informe oficial y los datos algorítmicos
2. Identifique los hallazgos más preocupantes para el ciudadano común
3. Proponga 3 recomendaciones concretas de seguimiento ciudadano
...
5. Mencione los mecanismos de control (CPCCS, Contraloría, Defensoría) cuando corresponda"""
                try:
                    ...
                    st.download_button(
                        ...
                        data=_resp_tribunal.text.encode(),
                        file_name="dictamen_tribunal_ia_montecristi_2024.txt",
                        use_container_width=True
                    )
                except Exception as _e:
                    st.error(f"Error al consultar Gemini: {_e}")
    else:
        st.info("💡 Activa **GEMINI_API_KEY** en los secrets de Streamlit para desbloquear el Tribunal IA.")
    # ────────────────────────────────────────────────────────
